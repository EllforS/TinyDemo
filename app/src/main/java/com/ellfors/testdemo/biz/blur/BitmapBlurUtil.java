...
public class BitmapBlurUtil
{
    ...
    {
        if (instance == null) synchronized (BitmapBlurUtil.class)
        {
            if (instance == null)
            {
     ...
     * @param imageView
     * @param bitmap
     * @param bitmap_scale 需要缩放的倍数
     *                     图片压缩后可加快速度
     * @param blur_radius  模糊度
     *                     越大越模糊
     */
    ...
    private Bitmap blur(Context context, Bitmap bitmap, float bitmap_scale, int blur_radius)
    {
        ...
        if (bitmap_scale < 1)
        {
            inputBitmap = Bitmap.createScaledBitmap(bitmap, Math.round(bitmap.getWidth() * bitmap_scale), Math.round(bitmap.getHeight() * bitmap_scale), false);
        }
        Bitmap outputBitmap;
        //rendScript只适用于Build.VERSION_CODES大于17
